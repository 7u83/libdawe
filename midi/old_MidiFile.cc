...
MidiFile::findNextTrack(std::ifstream *file)
{
	...
	while (!file->eof()) {
		...
		std::string chunkType = read(file, 4);
...
exit(0);
	...
	try {
		...
		for (unsigned track = 0; track < m_numberOfTracks; ++track) {
			...
			MidiEventArrayList * l = new MidiEventArrayList();
			tracks.push_back(l);

			parseTrack(midiFile,len,tracks[track]);
...
void
MidiFile::parseTrack(std::ifstream *midiFile, uint32_t len, MidiEventArrayList * track)
{
...
/*	printf("=== dump track ===\n");
	for(int i=0; i<track.size(); i++){
		printf("%d: ", track.getAtIndex(i)->getTime());
		track.getAtIndex(i)->dump();
	}
	//exit(0);
	printf("dump done\n");
*/	return;

	std::cout << "================ parse track\n";
	...
	//	std::cout << std::dec << "evt tme: " << eventTime << std::endl;
		...
		if (midiByte & MIDI_STATUS_BYTE_MASK) {
			RG_DEBUG << "parseTrack(): have new status byte" << std::hex << midiByte;
			...
			RG_DEBUG << "parseTrack(): using running status (byte " << std::hex << midiByte << " found)";
		}
		...
		if (statusByte == MIDI_FILE_META_EVENT) {
			...
			RG_DEBUG << "parseTrack(): Meta event of type " << std::hex << metaEventCode << " and " << messageLength << " bytes found";
			...
			MidiEvent *e = new MidiEvent(nextId(),eventTime,
...
//			allEvents.insert(*e);
			...
			if (e->isMeta()) {
				...
				// store it in the global tempo map
				if (e->getMetaEventCode() == MIDI_SET_TEMPO) {
					std::cout << "TEMPO TO TEMPOMAP:" << *e << std::endl;
//					this->tempoMap.insert(*e);
				}
			}
		...
		case MIDI_PITCH_BEND:
		{
			...
			// create and store our event
			MidiEvent *midiEvent =
					new MidiEvent(nextId(), eventTime, statusByte, data1, data2);
			m_midiComposition[trackNum].push_back(midiEvent);
//			allEvents.insert(*midiEvent);
		...
		{
			RG_DEBUG << "parseTrack(): Program change (Cn) or channel aftertouch (Dn): time "
				 << deltaTime << ", code " << std::hex << statusByte << std::dec <<
				    ", data " << (int) data1  << " going to track " << trackNum;

			// create and store our event
			MidiEvent *midiEvent =
					new MidiEvent(this->nextId(), eventTime, statusByte, data1);
			m_midiComposition[trackNum].push_back(midiEvent);
	//		allEvents.insert(*midiEvent);
		}
			break;
		...
		case MIDI_SYSTEM_EXCLUSIVE:
		{
			...
			// create and store our event
			MidiEvent *midiEvent =
					new MidiEvent(nextId(), eventTime,
		...
		default:
			RG_WARNING << "parseTrack() - Unsupported MIDI Status Byte:  " << std::hex << statusByte;
			break;
		}
...
bool
MidiFile::convertToRosegarden(const std::string &filename)
{
	m_nextId=1;
...
*/
	...
	/*
    FIXTHIS */
//	const int rosegardenPPQ = 960; // Note(Note::Crotchet).getDuration();
...
//	nmiditicks_T maxTime = 0;

	//  Segment *conductorSegment = 0; //0;
...
MidiFile::convertToMidi(RosegardenDocument *doc, const QString &filename)
{
    ...
    bool haveUI = (mainWindow != 0);

    SequenceManager *sequenceManager = 0;
